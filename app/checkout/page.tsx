...
//   const subtotal = cartItems.reduce((s:any,it:any) => s + Number(it.price) * Number(it.quantity), 0);
...
//         redirect_url: `${process.env.NEXT_PUBLIC_BASE_URL}/api/paystack/verify` // we will use verify endpoint
//       };
...
//       if (!res.ok) throw new Error(json?.message || 'Could not init payment');
...
//         {/* inputs (same as earlier) */}
//         <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
//           <div><input value={form.email} onChange={(e)=>setForm({...form,email:e.target.value})} className="border rounded p-3 bg-gray-100 w-full" placeholder="Email address *" /></div>
//           <div><input value={form.firstName} onChange={(e)=>setForm({...form,firstName:e.target.value})} className="border rounded p-3 bg-gray-100 w-full" placeholder="First name *" /></div>
//           <div><input value={form.lastName} onChange={(e)=>setForm({...form,lastName:e.target.value})} className="border rounded p-3 bg-gray-100 w-full" placeholder="Last name *" /></div>
//           <div><input disabled value="Nigeria" className="border rounded p-3 bg-gray-200 w-full" /></div>
//         </div>

//         <input value={form.state} onChange={(e)=>setForm({...form,state:e.target.value})} className="border rounded p-3 bg-gray-100 w-full mt-2" placeholder="State / County (optional)" />
//         <input value={form.city} onChange={(e)=>setForm({...form,city:e.target.value})} className="border rounded p-3 bg-gray-100 w-full mt-2" placeholder="City *" />
//         <input value={form.address} onChange={(e)=>setForm({...form,address:e.target.value})} className="border rounded p-3 bg-gray-100 w-full mt-2" placeholder="Street Address *" />
//         <input value={form.phone} onChange={(e)=>setForm({...form,phone:e.target.value})} className="border rounded p-3 bg-gray-100 w-full mt-2" placeholder="Phone *" />
//         <label className="flex items-center gap-2 mt-3"><input type="checkbox" checked={form.newsletter} onChange={(e)=>setForm({...form,newsletter:e.target.checked})} /> Subscribe to our newsletter</label>
//         <label className="flex items-center gap-2 mt-3"><input type="checkbox" checked={form.differentAddress} onChange={(e)=>setForm({...form,differentAddress:e.target.checked})} /> Ship to a different address?</label>
//         <textarea value={form.notes} onChange={(e)=>setForm({...form,notes:e.target.value})} className="border rounded p-3 bg-gray-100 w-full h-28 mt-2" placeholder="Order notes (optional)"></textarea>
//       </div>

//       <div className="bg-gray-100 border border-(--color-columbia-blue) rounded p-4 h-fit">
...
//         <button onClick={onPlaceOrder} disabled={loading} className="mt-4 w-full bg-(--color-navyBlue) text-white font-bold py-2 rounded">{loading ? 'Processing...' : 'Place Order'}</button>
//         <button onClick={()=>{ if(confirm('Clear cart?')) clearCart(); }} className="mt-2 w-full border border-red-500 text-red-500 py-2 rounded">Clear Cart</button>
//       </div>
//     </div>
//   );
// }
...
//   const supabase = createClient();
  ...
  const saveAddressesToSupabase = async (billing: any, shipping: any) => {
    ...
    // Optional: Save billing + shipping in separate table if you want later
  };

  //🔥 PAYMENT LOGIC// In CheckoutPage onPlaceOrder (client)
// const onPlaceOrder = async () => {
//   if (!validate()) return;
//   setLoading(true);
//   try {
...
//       items: cartItems,
//       amount: Math.round(subtotal * 100), // kobo (client-side snapshot)
//       currency: 'NGN',
//       user_id: user?.id ?? null,
//       redirect_url: `${process.env.NEXT_PUBLIC_BASE_URL}/api/paystack/verify`,
//     };
...
//     if (json.authorization_url) {
//       // store a small local mapping so after redirect we can clear client cart if needed
//       // localStorage.setItem('last_order_ref', json.order_ref);
//       window.location.href = json.authorization_url;
//     } else {
//       alert('Payment initiated but no redirect URL returned');
//     }
